...
cd /e/BaiduYunDownload/freetradechain/freetradechain
make distdir
mkdir -p win32-build
...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /e/BaiduYunDownload/freetradechain/freetradechain/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /e/BaiduYunDownload/freetradechain/freetradechain/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /e/BaiduYunDownload/freetradechain/freetradechain/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /e/BaiduYunDownload/freetradechain/freetradechain/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /e/BaiduYunDownload/freetradechain/freetradechain/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /e/BaiduYunDownload/freetradechain/freetradechain/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /e/BaiduYunDownload/freetradechain/freetradechain/linux-build
make check

# Run RPC integration test on Linux:
/e/BaiduYunDownload/freetradechain/freetradechain/qa/rpc-tests/wallet.sh /e/BaiduYunDownload/freetradechain/freetradechain/linux-build/src
/e/BaiduYunDownload/freetradechain/freetradechain/qa/rpc-tests/listtransactions.py --srcdir /e/BaiduYunDownload/freetradechain/freetradechain/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /e/BaiduYunDownload/freetradechain/freetradechain/win32-build
  make check
fi
